...
Все объекты данных должны быть помечены реализацией интерфейса-маркера (пустого интерфейса) [*IDataObject*](https://htmlpreview.github.io/?https://raw.githubusercontent.com/mickfierte/XData/master/docs/doc/Contents/1/55.html). Все объекты описывающие подчиненные запросы должны быть помечены реализацией интерфейса-маркера (пустого интерфейса) [*ISqlObject*](https://htmlpreview.github.io/?https://raw.githubusercontent.com/mickfierte/XData/master/docs/doc/Contents/1/60.html).
...
Для работы с большим бинарным объектом (BLOB) с использованием отложенного (lazy) связывания в XData предлагается использовать тип свойства [*Lob*](https://htmlpreview.github.io/?https://raw.githubusercontent.com/mickfierte/XData/master/docs/doc/Contents/2/199.html). Для этого типа определены следующие 
...
```
### Свойства типа - Xml
Для работы с Xml с использованием отложенного (lazy) связывания в XData предлагается использовать тип свойства [*Xml*](https://htmlpreview.github.io/?https://raw.githubusercontent.com/mickfierte/XData/master/docs/doc/Contents/2/200.html). Для этого типа определены следующие 
...
```
В результате доступ к значению свойства будет обычным, значение свойства будет получено с помощью метода расширения [*GetProperty*](https://htmlpreview.github.io/?https://raw.githubusercontent.com/mickfierte/XData/master/docs/doc/Contents/2/103.html), а изменить его будет невозможно.
...
Нужно отметить, что регистрация возможности (правил) установления внешней связи между двумя объектами описывается в описании объектного преобразования главного объекта (master), а когда будет установлена реальная связь между [репозиториями](./glossary.md#Репозиторий) с помощью метода [*GetChild*](https://htmlpreview.github.io/?https://raw.githubusercontent.com/mickfierte/XData/master/docs/doc/Contents/2/160.html) эти правила просто начнут работать.

Для разрыва отношений master-slave необходимо вызвать метод [*DetachChild*](https://htmlpreview.github.io/?https://raw.githubusercontent.com/mickfierte/XData/master/docs/doc/Contents/2/158.html). При закрытии [слоя](./glossary.md#Слои) отношения master-slave внутри него разрываются автоматически.

Изменение фильтров действующих на зависимые репозитории производится в момент установки текущего объекта в главном (или в нескольких из них в зависимости от бизнес логики) с помощью метода расширения интерфейса *IDataObject* [**SetCurrent**](https://htmlpreview.github.io/?https://raw.githubusercontent.com/mickfierte/XData/master/docs/doc/Contents/2/118.html).
```csharp
...
```
Кроме фильтрации зависимых репозиториев в пользовательском интерфейсе удобно использовать для проведения каскадного удаления данных:
```csharp
newInvoice.SetDeleted(true);
newInvoice.Submit(DataSubmitFlag.Cascade);
```
или каскадной очистки от предварительно отобранных по некоторому условию объектов:
```csharp
invoices.Clear(x => x.DocStateCode == "REJECTED", DataSubmitFlag.Cascade);
```
...
>Необходимо учитывать, что XData может именно автоматически вести таблицы иерархии при внесении изменений в основную таблицу, но при этом XData не может создавать таблицы иерархии и не может гарантировать консистентность таблицы иерархии при изменениях внесенных в БД минуя программные объекты с описанием иерархии.
...
>Тип отражаемый в UDT должен поддерживать Xml сериализацию стандартными методами
...
>Есть некоторая специфика работы c пользовательскими типами данных (UDT) совместно с ORACLE ODP провайдером в трех-звенной модели, которая описана в разделе [Частные случаи и советы - Особенности использования пользовательских типов данных совместно с ODP](./tips_and_tricks.md#Особенности-использования-пользовательских-типов-данных-совместно-с-odp). Просьба внимательно ознакомиться и принять к сведению.
