...
Изменение ограничений действующих на зависимые репозитории производится в момент установки текущего объекта в главном (или в нескольких из них в зависимости от бизнес логики) с помощью метода расширения интерфейса *IDataObject* *SetCurrent*.
```csharp
...
```
Кроме поддержки ограничений на объекты зависимость master-slave удобно использовать для проведения каскадного удаления данных:
```csharp
newInvoice.SetDeleted(true);
newInvoice.Submit(DataSubmitFlag.Cascade);
```
или кскадной очистки от предварительно отобранных по некоторому условию объектов:
```csharp
