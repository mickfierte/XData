...
Нужно отметить, что регистрация возможности (правил) установления внешней связи между двумя объектами описывается в описании объектного преобразования главного объекта (master), а когда будет установлена реальная связь между [репозиториями](./glossary.md#Репозиторий) с помощью метода *GetChild* эти правила просто начнут работать.

Для разрыва отношений master-slave необходимо вызвать метод *DetachChild*. При закрытии [слоя](./glossary.md#Слои) отношения master-slave внутри него разрываются автоматически.

Изменение фильтров действующих на зависимые репозитории производится в момент установки текущего объекта в главном (или в нескольких из них в зависимости от бизнес логики) с помощью метода расширения интерфейса *IDataObject* *SetCurrent*.
```csharp
